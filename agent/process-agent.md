...
1. **Offer recap.** If `Last Updated` is more than 2 days ago, ask: *"It's been [N] days since our last session. Would you like a quick recap of where we are?"* If the developer says yes, summarize the State Digest and key decisions. Do not re-load state files unless the recap reveals a gap.
